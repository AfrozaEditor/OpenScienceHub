...
export class IssuanceService {
  ...
  async getOfferStatus(credentialExchangeId: string) {
    ...
    const credentialRecord =
      await agent.didcomm.credentials.findById(credentialExchangeId);
  ...
  async createSchema(input: CreateSchemaInput) {
    ...
    const { getUnqualifiedSchemaId, parseIndySchemaId } =
      await import('@credo-ts/anoncreds');
    ...
    if (schemaResult.schemaState.state === 'failed') {
      throw new Error(
        `Schema creation failed: ${schemaResult.schemaState.reason}`,
      );
    }
  ...
  async createCredentialDefinition(input: CreateCredDefInput) {
    ...
    if (result.credentialDefinitionState.state === 'failed') {
      throw new Error(
        `Cred def creation failed: ${result.credentialDefinitionState.reason}`,
      );
    }
  ...
  async offerCredentialWithUrl(input: {
    ...
    const { message, credentialExchangeRecord: credentialRecord } =
      await agent.didcomm.credentials.createOffer({
    ...
    if (!credDef) {
      throw new Error(
        `Credential Definition not found for ID: ${input.credentialDefinitionId}`,
      );
    }
