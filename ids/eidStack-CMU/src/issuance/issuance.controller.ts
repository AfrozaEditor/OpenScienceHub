...
export class IssuanceController {
  ...
  async getOfferStatus(
    @Query('credentialExchangeId') credentialExchangeId: string,
  ) {
    return handleController(async () => {
      ...
      const state =
        await this.issuanceService.getOfferStatus(credentialExchangeId);
      return state;
    });
  }
  ...
  async listCredentialDefinitionsPaginated(
    ...
    return handleController(async () => {
      try {
        ...
        const res =
          await this.issuanceService.listCredentialDefinitionsPaginated(p, l);
        return { success: true, data: res };
      } catch (err) {
    ...
    return handleController(async () => {
      try {
        if (!body || !body.schemaId || !body.tag) {
          return {
            success: false,
            message: 'Invalid credential definition payload',
            statusCode: 400,
          };
        }
  ...
  async offerCredentialWithUrl(
    @Req() req: Request,
    @Body() body: OfferCredentialWithUrlDto,
  ) {
    const correlationId = req.headers['x-correlation-id'] as string | undefined;
    return handleController(async () => {
      try {
        if (!body || !body.credentialDefinitionId) {
          return {
            success: false,
            message: 'Missing credentialDefinitionId',
            statusCode: 400,
          };
        }
