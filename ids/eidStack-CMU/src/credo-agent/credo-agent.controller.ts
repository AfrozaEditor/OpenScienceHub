...
export class CredoAgentController {
  ...
  async receiveInvitation(@Body() body: ReceiveInvitationDto) {
    try {
      const res = await this.credoAgentService.receiveInvitation(
        body.invitationUrl,
      );
      ...
      try {
        if (res && typeof res === 'object') {
          ...
          if ('id' in res && typeof (res as any).id === 'string')
            id = (res as any).id;
          else if (
            'invitationId' in res &&
            typeof (res as any).invitationId === 'string'
          )
            id = (res as any).invitationId;
          else if (
            'connectionId' in res &&
            typeof (res as any).connectionId === 'string'
          )
        ...
        payload.debug = {
          rawType:
            res && typeof res === 'object'
              ? ((res as any).constructor?.name ?? 'object')
              : typeof res,
        };
      }
