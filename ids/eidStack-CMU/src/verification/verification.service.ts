...
export class VerificationService {
  ...
  //   const proof = await agent.didcomm.proofs.findById(proofRecordId);
  //   return proof?.state;
  // }
  ...
  async getProofStatus(proofRecordId: string) {
    ...
    const proofRecord = await agent.didcomm.proofs.findById(proofRecordId);
    ...
    const requestedAttrs = request?.anoncreds?.requested_attributes;
  ...
  async createProofRequest(input: {
    ...
    const oob = await agent.didcomm.oob.createInvitation({
