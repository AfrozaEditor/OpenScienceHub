...
export const getCredentials = async (
    agent: Agent
): Promise<{ record: DidCommCredentialExchangeRecord; connectionLabel?: string }[]> => {
    ...
    try {
        ...
        const [ credentialRecords, allConnections ] = await retryWithBackoff(async () => {
            return await Promise.all([
                (agent as any).credentials.getAll(),
                (agent as any).connections.getAll()
            ]);
        });
            ...
            credentialRecords.map(async (record) => {
                ...
                if (connectionLabel === 'Unknown Issuer') {
                    ...
                    if (outOfBandId && typeof outOfBandId === 'string') {
                        try {
                            const outOfBandRecord = await (agent as any).oob.findById(outOfBandId);
                            if (outOfBandRecord) {
...
export const acceptCredentialOffer = async (
    ...
    buildPreviewFn: (record: DidCommCredentialExchangeRecord) => Promise<CredentialOfferPreview>
): Promise<CredentialOfferPreview> => {
    if (!agent) throw new Error('Agent not initialized');
    try {
        console.log('Accepting credential offer:', credentialRecordId);
        const record = await (agent as any).credentials.acceptOffer({
            credentialRecordId,
        });
