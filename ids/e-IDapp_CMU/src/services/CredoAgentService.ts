...
import type { InitConfig } from './credoCore';
...
export default class CredoAgentService {
  ...
  private installDidCommCompatibilityAliases(agent: Agent): void {
    const agentWithModules = agent as any;
    const didcomm = agentWithModules.didcomm;

    if (!didcomm) {
      return;
    }

    agentWithModules.oob = didcomm.oob;
    agentWithModules.connections = didcomm.connections;

    const normalizeCredentialOptions = (options: any = {}) => ({
      ...options,
      credentialExchangeRecordId: options.credentialExchangeRecordId ?? options.credentialRecordId,
    });

    const normalizeProofOptions = (options: any = {}) => ({
      ...options,
      proofExchangeRecordId: options.proofExchangeRecordId ?? options.proofRecordId,
    });

    agentWithModules.credentials = new Proxy(didcomm.credentials, {
      get(target, property) {
        if (['acceptOffer', 'acceptCredential', 'acceptRequest', 'declineOffer'].includes(String(property))) {
          return (options: any) => (target as any)[property](normalizeCredentialOptions(options));
        }
    ...
    agentWithModules.proofs = new Proxy(didcomm.proofs, {
      get(target, property) {
        if (
          [
            'acceptRequest',
          ...
          return (options: any) => (target as any)[property](normalizeProofOptions(options));
        }
  ...
  async initialize(config?: CredoAgentConfig): Promise<{ success: boolean; agentId?: string; agentLabel?: string }> {
    try {
      ...
      this.agentLabel = config?.label || (await loadUserData())?.name || 'PolyID Holder';
      ...
      const modules: any = {
        ...
        didcomm: new DidCommModule({
          ...
          mediationRecipient: invitationUrl
            ? {
      ...
      this.installDidCommCompatibilityAliases(this.agent);
      ...
      try {
        const mediationRepository = this.agent.dependencyManager.resolve(DidCommMediationRepository);
  ...
  async getCredentials(): Promise<{ record: DidCommCredentialExchangeRecord; connectionLabel?: string }[]> {
    if (!this.isInitialized || !this.agent) throw new Error('Agent not initialized');
  ...
  async buildCredentialOfferPreview(record: DidCommCredentialExchangeRecord): Promise<CredentialOfferPreview> {
