...
export const mapConnectionRecord = (record: DidCommConnectionRecord | null | undefined, outOfBandRecord?: any, urlLabel?: any, urlType?: any): Connection => {
...
export const getConnections = async (agent: Agent): Promise<Connection[]> => {
    ...
    try {
        ...
        const allConnections = await retryWithBackoff(async () => {
            return await (agent as any).connections.getAll();
        });
            ...
            completedConnections.map(async (record) => {
                ...
                if (record.outOfBandId) {
                    try {
                        ...
                        outOfBandRecord = await (agent as any).oob.findById(record.outOfBandId);
                    } catch (error) {
...
export const declineConnection = async (agent: Agent, invitationUrl: string): Promise<void> => {
    ...
    try {
        ...
        const { outOfBandRecord, connectionRecord } = await (agent as any).oob.receiveInvitationFromUrl(
            invitationUrl,
        ...
        if (connectionRecord) {
            try {
                await (agent as any).connections.deleteById(connectionRecord.id);
        ...
        if (outOfBandRecord) {
            try {
                await (agent as any).oob.deleteById(outOfBandRecord.id);
