...
const CredentialEventTypes = {
    CredentialStateChanged: 'DidCommCredentialStateChanged',
} as const;
...
const handleOfferReceived = async (agent: Agent, recordId: string) => {
    ...
    const requestRecord = await (agent as any).credentials.acceptOffer({
...
const handleCredentialReceived = async (agent: Agent, recordId: string) => {
    ...
    const storedRecord = await (agent as any).credentials.acceptCredential({
        credentialRecordId: recordId
    });
...
const handleCredentialDone = async (agent: Agent, recordId: string, credentialRecord: any) => {
    ...
    try {
        const finalRecord = await (agent as any).credentials.getById(recordId);
...
export const setupCredentialEventListener = (agent: Agent) => {
    ...
    agent.events.on(
        ...
        async (event: any) => {
            const credentialRecord = (event as any).payload.credentialRecord ?? (event as any).payload.credentialExchangeRecord;
...
export const setupProofEventListener = (agent: Agent) => {
    ...
    agent.events.on(
        ...
        async (event: any) => {
            const { proofRecord } = (event as any).payload;
            ...
            try {
                switch (proofRecord.state) {
                    case ProofState.RequestReceived:
                        ...
                        const formatData = await (agent as any).proofs.getFormatData(proofRecord.id);
...
export const setupConnectionEventListener = (agent: Agent) => {
    ...
    agent.events.on(
        ...
        async (event: any) => {
            const { connectionRecord } = (event as any).payload;
