...
export const getProofRecord = async (agent: Agent, proofRecordId: string) => {
    return await (agent as any).proofs.getById(proofRecordId);
};
...
export const getMatchingCredentialsForProofRequest = async (
    ...
    try {
        ...
        const retrievedCredentials = await (agent as any).proofs.getCredentialsForRequest({
            proofRecordId,
        });
...
export const acceptProofRequest = async (agent: Agent, proofRecordId: string) => {
    try {
        ...
        const proofRecord = await (agent as any).proofs.getById(proofRecordId);
        ...
        const requestedCredentials = await (agent as any).proofs.selectCredentialsForRequest({
            proofRecordId,
        });
        ...
        const result = await (agent as any).proofs.acceptRequest({
            proofRecordId,
...
export const declineProofRequest = async (agent: Agent, proofRecordId: string) => {
    await (agent as any).proofs.declineRequest({ proofRecordId });
};
